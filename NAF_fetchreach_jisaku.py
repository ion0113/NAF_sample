...
class Agent:
    def __init__(self, env, learning_rate=1e-3, state_dim=13, action_dim=4, hidden_size_v=16, hidden_size_mu=16, hidden_size_l=32, test=False):#state_dim=13, action_dim=4
        self.env = env
        ...
        self.hidden_size_mu = hidden_size_mu
        ...
        if not test:
            ...
            self.t_net_q.load_weights("weight_fetchreach.h5")
    ...
    def createModel(self):
        ...
        # < Value function >
        V = Dense(self.hidden_size_v, kernel_initializer=init)(x)
        V = BatchNormalization()(V)
        V = Activation("relu")(V)
        V = Dense(self.hidden_size_v, kernel_initializer=init)(V)
        V = BatchNormalization()(V)
        V = Activation("relu")(V)
        ...
        # < Action Mean >
        mu = Dense(self.hidden_size_mu, kernel_initializer=init)(x)
        mu = BatchNormalization()(mu)
        mu = Activation("relu")(mu)
        mu = Dense(self.hidden_size_mu, kernel_initializer=init)(mu)
        mu = BatchNormalization()(mu)
        mu = Activation("relu")(mu)
        ...
        l0 = Dense(self.hidden_size_l, kernel_initializer=init)(x)
...
def train():    
    ...
    agent = Agent(env=env, learning_rate=lr, hidden_size_v=hidden_v, hidden_size_mu=hidden_mu, hidden_size_l=hidden_l)
    # リプレイメモリ
    replay_memory = deque()
    ...
    for episode in range(n_episode):
        ...
        while not end_flag:
            # 行動にノイズを付与
            noize = np.random.normal(loc=0, scale=sigma, size=(1,4))
            #print(noize)
        ...
        if episode != 0 and episode % 4 == 0:
            ...
            agent.t_net_q.save_weights("weights/weight_fetchreach.h5")

    env.close()

    agent.WeightCopy()
    # Q-networkの重みをTarget-networkにコピー
    agent.t_net_q.save_weights("weight_fetchreach.h5")


def test():
    env = gym.make('FetchReach-v1', reward_type='dense') # 環境
    agent = Agent(env=env, learning_rate=lr, hidden_size_v=hidden_v, hidden_size_mu=hidden_mu, hidden_size_l=hidden_l, test=True)
    episode = 0
    ...
    while 1:
        ...
        while not end_flag:
            ...
            env.render()
        print('Test:episode = %3d, end_reward =%f, sum_reward=%d' % (episode, reward, sum_reward))
